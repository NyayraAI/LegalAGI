...
def clear_cache() -> None:
    ...
    try:
        if redis_client:
            ...
            logger.info(
                f"✅ Cleared {len(embed_keys)} embedding keys and {len(match_keys)} match keys"
            )
        else:
            memory_cache.clear()
    ...
    try:
        if redis_client:
            ...
            return {
                ...
                "embedding_keys": len(
                    [k for k in memory_cache.keys() if k.startswith("embed:")]
                ),
                "match_keys": len(
                    [k for k in memory_cache.keys() if k.startswith("match:")]
                ),
            }
